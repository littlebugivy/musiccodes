...
function Client(socket) {
	...
	// 	console.log("HAVE JOINED ROOMSL "+rooms);
	// })
	// io.in('visualRoom').emit('muziMsg', 'SEND MESSAGE: MUZI --> VISUAl');
  
	// io.of('/').adapter.remoteJoin('5q3HuZRE_b9ImGEFAAAA', 'visualRoom', function (err) {
	// if (err) { /* unknown id */ console.log("Can't get room: visualRoom")}
	// // success
	// });
	...
	for (var li in lines) {
		lines[li].replace(/\s*(\d+(\.\d+)?):\s*(\d+(\.\d+)?)\s+(\d+)\s(\S+)\s*(\S+)?/,
			function (m, time, t2, freq, f2, velocity, note, off) {
				values.push({
					time: Number(time), freq: Number(freq), velocity: Number(velocity),
...
io.on('connection', function (socket) {
	...
	socket.on('visualMsg', function (data) {
		...
		io.to('visualRoom').emit('visualMsg', 'From muzi-backend');
	})
	// io.on('muziMsg', function (data) {
	// 	console.log('GET A MESSAGE FROM MUZICODES!!' + data);
	// })
